...
const FIREBASE_URL = "https://smart-home-69271-default-rtdb.firebaseio.com/devices/esp32_smart_home_1";
...
app.get("/auth", (req, res) => {
  ...
  console.log("OAuth /auth redirecting to:", responseUrl);
...
app.post("/smarthome", async (req, res) => {
  ...
  console.log(`Received Smart Home Intent: ${intent}`);
  ...
  if (intent === "action.devices.QUERY") {
    try {
      ...
      console.error("Error query Firebase:", e);
        ...
        for (const execution of executions) {
          ...
          if (commandObj) {
            try {
              ...
              await fetch(`${FIREBASE_URL}/commands.json`, {
                method: "PUT",
...
app.listen(PORT, () => {
  console.log(`Local Google Home Action Webhook listening on port ${PORT}`);
});
