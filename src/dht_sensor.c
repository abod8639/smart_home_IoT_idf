...
esp_err_t dht_sensor_read(float *temperature, float *humidity) {
    ...
    for (int i = 0; i < 40; i++) {
        ...
        if (dht_await_level(1, 65) < 0) {
            ESP_LOGW(TAG, "DHT22 bit %d LOW timeout", i);
            return ESP_ERR_TIMEOUT;
        }
        ...
        if (dht_await_level(0, 80) < 0) {
            ESP_LOGW(TAG, "DHT22 bit %d HIGH timeout", i);
            return ESP_ERR_TIMEOUT;
        }
    }
    ...
    if (checksum != data[4]) {
        ESP_LOGW(TAG, "DHT22 checksum error (got 0x%02X, expected 0x%02X)", data[4], checksum);
